...
def extraer_dia(valor_fecha):
    ...
    if isinstance(valor_fecha, datetime):
        return str(valor_fecha.day)
    elif isinstance(valor_fecha, str) and "/" in valor_fecha:
        return valor_fecha.split("/")[0]
...
def extraer_mes(valor_fecha):
    if valor_fecha is None:
        return ""
    if isinstance(valor_fecha, datetime):
        return MESES_ES.get(valor_fecha.month, "")
    elif isinstance(valor_fecha, str) and "/" in valor_fecha:
        partes = valor_fecha.split("/")
        if len(partes) >= 2:
            try:
                return MESES_ES.get(int(partes[1]), "")
            except ValueError:
                return ""
    return ""
...
def tiene_color(celda):
    if celda.fill and celda.fill.patternType == 'solid':
        ...
        if color not in ['00000000', 'FFFFFFFF', 0, '00FFFFFF']:
            return True
    return False
...
def process():
    ...
    try:
        ...
        for nombre_hoja in ['Data', 'Ingresos']:
            if nombre_hoja in wb_origen.sheetnames:
                ...
                for fila in range(2, hoja.max_row + 1):
                    ...
                    if tiene_color(celda_c) or tiene_color(celda_f) or tiene_color(celda_j):
                        continue
                    ...
                    if nombre:
                        lista_principal.append((
                            str(nombre).strip(),
                            str(extraer_dia(celda_f.value)),
                            extraer_mes(celda_f.value),
                            str(celda_j.value or '').strip()
                        ))
        ...
        if 'Ceses' in wb_origen.sheetnames:
            ...
            for fila in range(2, hoja_ceses.max_row + 1):
                ...
                if nombre:
                    ...
                    lista_ceses.append((
                        str(nombre).strip(),
                        str(extraer_dia(fecha_c)),
                        extraer_mes(fecha_c),
                        str(hoja_ceses.cell(row=fila, column=10).value or '').strip()
                    ))

        lista_final = [r for r in lista_principal if r not in lista_ceses]

        # --- Construir Excel estilizado en memoria ---
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name='Resultado_Patron.xlsx'
        )
